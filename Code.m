...
numClasses = length(unique(GroupTrain));
...
for j=1:size(TestSet,1)
    predicted = zeros(1,numClasses);
    for k=1:numClasses
        predicted(k) = predict(models{k}, TestSet(j,:));
    end
    [~, result(j)] = max(predicted);
end
...
result = predict(SVMStruct, imTest);
if(result==1)
    msgbox('Desert');
else
    msgbox('Forest');
